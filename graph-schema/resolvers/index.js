...
export function change_address( args, context, graphql) {
  ...
  return party_db.any('update contact_mechanism set end_point = $1, directions = $2 where id = $3', [modified_address.street_address, modified_address.directions, modified_address.id])
  .then(() => ({id: modified_address.id}));
}

export function create_address(args, context, graphql) {
  let party_db = context.party_db;
  let new_address = args.new_address;
  return party_db.one('insert into contact_mechanism (end_point, directions, contact_mechanism_type_id) values( $1, $2, $3) returning id', [new_address.street_address, new_address.directions, context.contact_mechanism_types.get('Postal Address')])
  .then(result => {
    ...
    let country = party_db.none("insert into contact_mechanism_geographic_boundary (contact_mechanism_id, geographic_boundary_id) values ($1, $2)", [result.id, new_address.country_id]);
